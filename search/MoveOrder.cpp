...
void sortCapturesByMVVLVA(const ChessBoard& board, MoveList& moveList, int startIndex, int endIndex) {
    // sorts the captures from indices startIndex (inclusive) to endIndex (exclusive) by MVV-LVA.
    std::array<int8_t,4096> moveScores;
    for (int i = startIndex; i <= endIndex; i++) {
        move_t move = moveList.at(i);
        moveScores[move >> 4] = getCaptureMVVLVAScore(board,move);
    }

    // Now we do insertion sort
    for (int newIndex = startIndex + 1; newIndex < endIndex; newIndex++) {
        move_t thisMove = moveList.at(newIndex);
        int8_t thisMoveScore = moveScores[thisMove >> 4];
        int insertIndex;
        for (insertIndex = newIndex - 1; insertIndex >= startIndex; insertIndex--) {
            move_t newMove = moveList.at(insertIndex);
            int8_t newMoveScore = moveScores[newMove >> 4];
            if (thisMoveScore > newMoveScore)
                moveList.moveList[insertIndex + 1] = moveList.at(insertIndex);
            else
                break;
        }
        moveList.moveList[insertIndex + 1] = thisMove;
    }
}
...
void sortMoves(MoveList& legalMoves, const ChessBoard &board, move_t hashMove, const TwoKillerMoves &killerMoves,
    ...
    unsigned int sortedIndex = legalMoves.at(0) == hashMove; // which should be true unless we have no TT move
    const int goodCapturesStart = sortedIndex;
    ...
    const int goodCapturesEnd = sortedIndex;
    ...
    // Step 6: Sort captures by MVV-LVA
    sortCapturesByMVVLVA(board, legalMoves, goodCapturesStart, goodCapturesEnd);
}