...
bitboard_t getKingAttackedSquares (int square);
bitboard_t getKnightAttackedSquares (int square);
bitboard_t getBishopAttackedSquares (int square);
...
constexpr const static bitboard_t rook_magics[64] = {
...
// bishop magic number
constexpr const static bitboard_t bishop_magics[64] = {
...
constexpr const static bitboard_t ROOK_RELEVANT_BLOCKERS[64] = {282578800148862,
                                                         565157600297596,
                                                         1130315200595066,
                                                         2260630401190006,
                                                         4521260802379886,
...
constexpr const static bitboard_t BISHOP_RELEVANT_BLOCKERS[64] = {18049651735527936,
                                                           70506452091904,
                                                           275415828992,
                                                           1075975168,
                                                           38021120,
...
std::vector<bitboard_t> getKingAttackedSquaresTable ();
std::vector<bitboard_t> getKnightAttackedSquaresTable ();
std::vector<bitboard_t> getAllSubBitsOf (bitboard_t original);
std::vector<bitboard_t> getRookMagicBitboardTable(int startSquare, bitboard_t magicNumber, int numBits);
std::vector<bitboard_t> getBishopMagicBitboardTable(int startSquare, bitboard_t magicNumber, int numBits);
std::vector<std::vector<bitboard_t>> getRookMagicBitboardTable();
std::vector<std::vector<bitboard_t>> getBishopMagicBitboardTable();

const std::vector<std::vector<bitboard_t>> ROOK_MAGIC_BITBOARD_TABLE = getRookMagicBitboardTable();
const std::vector<std::vector<bitboard_t>> BISHOP_MAGIC_BITBOARD_TABLE = getBishopMagicBitboardTable();
const std::vector<bitboard_t> KING_ATTACKED_SQUARES_TABLE = getKingAttackedSquaresTable();
const std::vector<bitboard_t> KNIGHT_ATTACKED_SQUARES_TABLE = getKnightAttackedSquaresTable();

/////////////////////////////////////////////////////////////////////////////////////
/////////////////////           Part 4: Magic lookups           /////////////////////
/////////////////////////////////////////////////////////////////////////////////////

inline bitboard_t getMagicKingAttackedSquares (const int startingSquare) {
...
inline bitboard_t getWhitePawnAttackedSquares(const int startingSquare) {
    using namespace bitmasks;
    return ((512ULL << startingSquare) & NOT_A_FILE) | (((1ULL << (startingSquare)) >> 7) & NOT_H_FILE);
}

inline bitboard_t getBlackPawnAttackedSquares(const int startingSquare) {
    using namespace bitmasks;
    return ((128ULL << startingSquare) & NOT_A_FILE) | (((1ULL << (startingSquare)) >> 9) & NOT_H_FILE);
}

inline bitboard_t getMagicWhiteAttackedSquares (const int pieceType, const int startingSquare, const bitboard_t allPieces) {
    switch(pieceType) {
...
inline bitboard_t getMagicBlackAttackedSquares (const int pieceType, const int startingSquare, const bitboard_t allPieces) {
    switch(pieceType) {
        case QUEEN_CODE: return calcMagicQueenAttackedSquares(startingSquare,allPieces);
        case ROOK_CODE: return getMagicRookAttackedSquares(startingSquare,allPieces);
        case BISHOP_CODE: return getMagicBishopAttackedSquares(startingSquare,allPieces);
